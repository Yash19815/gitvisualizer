...
function App() {
  ...
  const handleLeftResizeEnd = useCallback(() => {
    localStorage.setItem('gitvisualizer-left-panel-width', leftPanelWidth.toString());
  }, [leftPanelWidth]);

  const handleRightResizeEnd = useCallback(() => {
    localStorage.setItem('gitvisualizer-right-panel-width', rightPanelWidth.toString());
  }, [rightPanelWidth]);
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-100 dark:bg-gray-950">
        ...
        <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Git Visualizer</h1>
            </div>
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        {repository && !isFullscreen && (
          <>
            <aside
              ...
              className="bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex-shrink-0"
            >
              <BranchList />
            </aside>
            <ResizeHandle side="left" onResize={handleLeftResize} onResizeEnd={handleLeftResizeEnd} />
          </>
        )}
        ...
        {repository && !isFullscreen && (
          <>
            <ResizeHandle side="right" onResize={handleRightResize} onResizeEnd={handleRightResizeEnd} />
            {selectedSubmodule ? (
              <SubmoduleDetails width={rightPanelWidth} />
            ) : (
              <aside
                style={{ width: rightPanelWidth }}
                className="bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-800 flex-shrink-0"
              >
                <CommitDetails />
              </aside>
            )}
            ...
            <span>
              {repository ? (
                <>
                  ...
                  {' '}commits from <strong className="text-gray-700 dark:text-gray-300">{repository.name}</strong>
                  {loadMode !== 'full' && (
